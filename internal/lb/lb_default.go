...
type DefaultProxyServer struct{}

func (d *DefaultProxyServer) newBalancer(targets []*middleware.ProxyTarget) middleware.ProxyBalancer {
	return middleware.NewRoundRobinBalancer(targets)
}

func (d *DefaultProxyServer) StartReverseProxy(e *echo.Echo, region string) {
	...
	balancer := d.newBalancer(targets)
